...
function ProjectEditor({ project, onBack, onSave, isSettingsMode = false }) {
  ...
  const droneVideoManagement = useDroneVideoManagement(project?.droneVideos || []);
  ...
  useEffect(() => {
    if (project) {
      setFormData({
        ...
        projectType: project.projectType || 'roads',
        // Поля для дорог
        roadLength: project.roadLength || '',
        roadCategory: project.roadCategory || '',
        surveyStartDate: project.surveyStartDate || '',
        surveyEndDate: project.surveyEndDate || ''
      });
